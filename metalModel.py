...
# and the error loss function for each batch size over time - for the metalloids found in salmon
...
data = pd.read_csv('albacore_metal.dat')
data = data.values
x = data[:, 1]
y = data[:, 2]
...
# rmse plots for each batch size
...
print("test on cadmium & mercury regression model")
...
plt.xlabel('cadmium amounts')
plt.ylabel('mercury amounts')
plt.title('cadmium & mercury cost optimization model')
...
plt.xlabel('cadmium amounts')
plt.ylabel('mercury amounts')
plt.title('cadmium & mercury cost optimization model - normal linear regression')
plt.xlim(0,x.max())
plt.ylim(0,y.max())
plt.scatter(x, y)
...
print("end of test on cadmium & mercury regression model")

print("test on cadmium & lead regression model")
...
plt.xlabel('cadmium amounts')
plt.ylabel('lead amounts')
plt.title('cadmium & lead cost optimization model')
...
print("end of test on cadmium & lead regression model")

print("test on mercury & lead regression model")
...
plt.xlabel('mercury amounts')
plt.ylabel('lead amounts')
plt.title('mercury & lead cost optimization model')
...
plt.xlabel('mercury amounts')
plt.ylabel('lead amounts')
plt.title('mercury & lead cost optimization model - normal linear regression')
plt.xlim(0,x.max())
plt.ylim(0,y.max())
plt.scatter(x, y)
plt.plot([min(x), max(x)], [min(y), max(y)], color='red')  # regression line
...
print("end of test on mercury & lead regression model")